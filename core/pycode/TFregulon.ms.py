...
def Generate_Expression_loom():
    ...
    elif args.annot.split('/')[-1]=='GSE60361_C1-3005.annotations.tsv':
        sc.pp.filter_cells(adata, min_genes=3500)
...
def Net_Infer():
   ...
   if args.species=='mouse':
      order='pyscenic grn --num_workers '+ args.cpu + ' --output '+ os.path.join(args.outdir,'02.Network.Adj.tsv') + ' --method grnboost2 '+ os.path.join(args.outdir,'01.Adata.Expression.loom') +' '+ os.path.join(args.tfdataset,'mm_mgi_tfs.txt') 
        ...
        --annotations_fname  '+ os.path.join(args.tfdataset,'motifs-v9-nr.hgnc-m0.001-o0.0.tbl') +' \
         --expression_mtx_fname '+os.path.join(args.outdir,'01.Adata.Expression.loom') +' \
           ...
           --num_workers '+args.cpu+'  \
            --mask_dropouts'
    if args.species=='mouse':
        order='pyscenic ctx '+os.path.join(args.outdir,'02.Network.Adj.tsv')+' '+os.path.join(args.tfdataset,'mm10__refseq-r80__10kb_up_and_down_tss.mc9nr.feather ') +' \
        --annotations_fname  '+ os.path.join(args.tfdataset,'motifs-v9-nr.mgi-m0.001-o0.0.tbl') +' \
         --expression_mtx_fname '+os.path.join(args.outdir,'01.Adata.Expression.loom') +' \
           --mode "dask_multiprocessing" \
           --output '+os.path.join(args.outdir,'03.Regulon.Refine.csv')+'  \
           --num_workers '+args.cpu+'  \
...
def Plot_RSS_Group(rss):
    ...
    for c,num in zip(cats, range(1,len(cats)+1)):
        plt.style.use('classic')
        x=rss.T[c]
        ax = fig.add_subplot(ceil(len(rss)/4),4,num)
...
修改为：输出top5RSS的regulon热图
'''
def Regulon_heatmap():
    plt.rcParams['figure.figsize'] = (6, 14)    
    
    ##输出每个cluster top 5 regulon的热图(有重复会去重)  
    ...
    regulon_list=list()
    for i in range(len(rss_cluster.index)):
        df1=pd.DataFrame(rss_cluster.iloc[i])
        df1=df1.sort_values([rss_cluster.index[i]],ascending=False).iloc[0:5]
        TF_gene=df1.columns
        regulon_list=regulon_list+list(df1.index)
    regulon_list=list(set(regulon_list))
    rss_cluster=rss_cluster[regulon_list]
    ...
    sns.heatmap(df_heatmap.T,ax=ax1,annot=False, linewidths=.7, cbar=True, cbar_kws={"shrink": .45,'label':'RSS'},square=False, linecolor='gray',
...
def Specify_Regulon_heatmap():
    ...
    TFreg.columns=['TFreg']
    TFreg=TFreg.drop_duplicates(['TFreg'])
...
def Plot_Certain_Regulon_AUC_umap(tfname):
   ...
   # pdf= PdfPages('D:/BGI/04.pipeline/test.data.2/tsne.pdf')
    ...
    sc.tl.umap(adata)
    sc.pl.umap(adata, color=['CellType',regulon_name],use_raw=False,cmap='coolwarm', legend_fontsize=12) 
    plt.tight_layout()
