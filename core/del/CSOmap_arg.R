...
# load the parameter
wkd <- args[1] 
TPM <- read.table(gzfile(args[2]), header = TRUE, check.names = FALSE, sep = "\t")
labelData <- read.table(args[3], header = TRUE, check.names= FALSE, sep = "\t")
colnames(labelData) <- c("cells", "labels", "fine")
LR <- read.table(args[4], header = FALSE, check.names = FALSE, sep = "\t")
outPath <- wkd 
...
# create output path
#dir.create(paste0(outPath, "/results"))
...
#coordsPath <- paste0(outPath, "/results/coordinates.txt")
coordsPath <- paste0(outPath, "/coordinates.txt")
...
# write counts
#countsPath <- paste0(outPath, "/results/counts.txt")
countsPath <- paste0(outPath, "/counts.txt")
...
# write p and q value
#pvaluePath <- paste0(outPath, "/results/pvalue.txt")
pvaluePath <- paste0(outPath, "/pvalue.txt")
#qvaluePath <- paste0(outPath, "/results/qvalue.txt")
qvaluePath <- paste0(outPath, "/qvalue.txt")
