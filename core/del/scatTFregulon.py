...
def Generate_Expression_loom():
    df_annot = pd.read_csv(args.annot)
    df_annot.columns=['CellID','CellType']
    df_tpm = pd.read_csv(args.genematrix, index_col=0)
...
def Certain_TF_regulon(tfname):
    ...
    adjacencies = pd.read_csv(os.path.join(args.outdir,args.net), index_col=False, sep='\t')
    lf = lp.connect(os.path.join(args.outdir,args.ScenicLoom), mode='r', validate=False)
...
def Plot_Certain_Regulon_AUC_umap(tfname):
   ...
   # pdf= PdfPages('D:/BGI/04.pipeline/test.data.2/tsne.pdf')
    adata=sc.read_h5ad(os.path.join(args.outdir,args.h5ad))
    plt.rcParams['figure.figsize'] = (15, 10)
