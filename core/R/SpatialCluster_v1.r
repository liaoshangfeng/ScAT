...
bayes_spatial_cluster <- function(){
    ...
    out.df = colData(sce)
    write.table(out.df, file=file.path(opts$outdir,'02.cluster.info.txt'), quote=F, row.names=T, col.names=T, sep="\t")
...
bayes_cluster_figure <- function(point_size = 1){
  df <- read.table(file.path(opts$outdir, '02.cluster.info.txt'), header = T, row.names = 1)
