...
gem_to_seuratObject <- function(data, prefix = 'sample', binsize = opts$binsize){
    ...
    # # 将Visium替换成slideSeq,便于后面直接绘图
    # seurat_spatialObj@images[['slice1']] = new(Class = 'SlideSeq', 
    #                                                assay = "Spatial", 
    #                                                key = "image_",
    #                                                coordinates = seurat_spatialObj@images[['slice1']]@coordinates[,c('row', 'col')])
...
preview_seuratObject <- function(sce, opts){
    ...
    p <- suppressWarnings(VlnPlot(sce, group.by = 'orig.ident', features = c(nFeature, nCount, 'percent.mt', 'percent.ribo', 'percent.hb', 'percent.plat'), pt.size = 0, ncol = 3)) + NoLegend()
    print (p)
...
if ((! is.null(opts$input)) & (! is.null(opts$out)) ){
    ...
    if (!file_test('-f', opts$input)| (opts$fileType == 'matrix' & suffix == 'gz') ) {
        ...
        if (file_test('-d', opts$input)){
            print('loading file from directory')
