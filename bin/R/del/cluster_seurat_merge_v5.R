...
DoSeuratProcess <- function(opts){
    ...
    if (DefaultAssay(sce) == 'SCT'|DefaultAssay(sce) == 'integrated'){
        ...
        if (length(sce@images) == 1){
            ...
            # p1  <- DimPlot(sce, reduction = "umap", cols = colorScheme)
            p2 <- SpatialPlot(sce, group.by = "seurat_clusters", cols = colorScheme, pt.size.factor = opts$ptsize)  + theme(legend.position = "right") 
            print(p2)
            while (!is.null(dev.list()))  dev.off() 
            gc()
            ...
            if (opts$mSpatialVar){
                ...
                for (i in top.features){
                    ...
                    p <- SpatialFeaturePlot(sce, features = i, pt.size.factor = opts$ptsize, alpha = c(0.1, 1))
                    print (p)
                    while (!is.null(dev.list()))  dev.off() 
                    # release the memory
                    gc()
