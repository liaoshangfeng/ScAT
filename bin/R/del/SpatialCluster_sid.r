...
input_format <- function(){
    ...
    counts = as(Stdata@assays$RNA@counts, "dgCMatrix")
...
bayes_cluster_figure <- function(point_size = 1){
  df <- read.table(file.path(opts$outdir, '02.cluster.info.txt'), header = F, row.names = 1)
  ...
  p0 <-
    ggplot(df, aes(x = x, y = y, color = cluster2)) +
  ...
  pdf(file = file.path(opts$outdir,'03.BayesSpace.spatial.cluster.pdf'), width = 6, height = 6)
  p0
  dev.off()
...
## update:2022/07/12
## rewrite by liaoshangfeng@genomics.cn
## 将seurat对象转成singleCellExperiment对象，后者是bayesSpace的输入文件
seuratObj_to_sceObj <- function(file_in){
    seuratObj =readRDS(file_in)
    df=data.frame(X=seuratObj@meta.data$x,Y=seuratObj@meta.data$y)
        df$spot_id=paste(df$X, df$Y, sep="X")
